...
describe('Teste de BetService', () => {
    ...
    it('#A1 - BetService - get last bet with empty database', async () => {
        let bet = await new BetService()
    ...
    it('#A2 - BetService - get all bets with empty database', async () => {
        let bet = await new BetService()
    ...
    it('#B1 - BetService - create a new bet', async () => {
    ...
    it('#B2 - BetService - create two equal bets', async () => {
        let bet = await new BetService( new Date(), 1010, 1010)
        let resultNewBet = await bet.NewBet()
        assert.equal(resultNewBet.status, 200, 'status deveria ser 200')
        assert.hasAllKeys(resultNewBet, ['status', 'payload'], 'result deveria ter as keys status e payload')

        let bet2 = await new BetService( new Date(), 1010, 1010)
    ...
    it('#C1 - BetService - get last bet with some records', async () => {
        let bet = await new BetService()
        let resultBet = await bet.getLastBet()
    ...
    it('#C1 - BetService - get all bets with some records', async () => {
        let bet = await new BetService()
        let resultBet = await bet.getBets()
        assert.equal(resultBet.status, 200, 'status deveria ser 200')
        assert.hasAllKeys(resultBet, ['status', 'payload'], 'result deveria ter as keys status e payload')
